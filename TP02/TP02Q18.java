...
class TP02Q18 {
    ...
    public static void sortname(Jogador[] array,int n) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            for (int j = i + 1; j < n; j++) {
                if(array[j].getEstadoNasc()=="Argentina"){
                    array[n] = array[j];
                }
                if(array[j] != null){
    ...
    public static void quicksort(Jogador [] array, int esq, int dir) {
        int i = esq, j = dir;
        Jogador pivo = array[(dir+esq)/2];
        while (i <= j) {
            while (array[i].getEstadoNasc().compareTo(pivo.getEstadoNasc()) < 0) i++;
            while (array[j].getEstadoNasc().compareTo(pivo.getEstadoNasc()) > 0) j--;
            if (i <= j) {
                Jogador temp = array[i];
                array[i] = array[j];
                array[j] = temp;
...
// (array[i].getNome().compareTo(pivo.getNome()) < 0)
    ...
    public static void main(String[] args) {
        ...
        Jogador[] arrJogador = new Jogador[463];
        int aux = 0;
        ...
        Jogador [] arrayQuick = new Jogador[10];
        // ordenando array
        
        quicksort(arrJogador, 0, aux-1);

        //sortname(arrJogador,aux);
        for (int i = 0; i < 10; i++){
            
            arrayQuick[i] = arrJogador[i];
        }

        
        sortname(arrayQuick,10);
        for (int j = 0; j < 10; j++){
            if(arrayQuick[j] != null){
            MyIO.println(arrayQuick[j].toString());
            }
