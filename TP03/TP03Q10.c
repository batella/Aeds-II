...
void Mostrar(int size, Celula *ultimo)
{
    if (ultimo->low != NULL)
    {
        Mostrar(size - 1, ultimo->low);
    }

    printf("[%d]", size);
    print(ultimo->jogador);
}
...
void MostrarPilha(Pilha pilha)
{
    ...
    Mostrar(pilha.size - 1, pilha.ultimo);
}


void Close(Pilha *Pilha)
{
...
Pilha newPilha()
{
    ...
    Pilha.Inserir = InserirFim;

    Pilha.Remover = RemoverFim;
